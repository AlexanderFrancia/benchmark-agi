...
import threading, uuid, math
from django.utils import timezone

from .models import EvaluationRun, EvaluationResult
from .core.lmstudio import list_models, chat_completion, LMStudioError
from .core.parsers import extract_matrix
...
from .services import arc_evaluator
...
def arc_index(request):
    # Listado de tareas
    tasks = arc_list(max_items=100000)
    ...
    return render(
        request,
        "arc_list.html",  # ← usamos el template que sí tienes
        {
            "tasks": tasks,
            "models": models,
...
def _cache_key(job_id: str) -> str:
    return arc_evaluator.CACHE_PREFIX + str(job_id)

@require_POST
def arc_evaluate_dataset(request):
    """
    ...
    tasks = arc_list(max_items=100000)  # usa tus directorios train/ y evaluation
    ...
    return render(request, "arc_evaluate_dataset.html", ctx)

def arc_run_live_page(request, run_id):
    run = get_object_or_404(EvaluationRun, pk=run_id)
    return render(request, "arc_run_live.html", {"run": run})


def arc_run_status(request, run_id):
    run = get_object_or_404(EvaluationRun, pk=run_id)
    results = (EvaluationResult.objects
               .filter(run=run)
               .order_by("task_id", "idx"))[:5000]
    return render(request, "arc_run_status.html", {"run": run, "results": results})

@require_POST
def start_arc_dataset_live(request):
    """
    ...
    if not model_id:
        ...
        return render(request, "arc_evaluate_dataset.html", {
    ...
    return redirect("arc_run_live", run_id=run.id)
